...
import com.kh.daeng.config.util.Smiliarity;
...
public class TourServiceImpl implements TourService {
	...
	@Autowired
	private Smiliarity similarity;

	// 추가
	@Override
	public List<Tour> getJaccard(int userNo) throws Exception {
		int pf_no = mapper.getUserPrNo(userNo);		 
		Preference user_pf = mapper.getUserPre(pf_no);
		List<Tour> tours = mapper.getAllTours();
		List<Tour> result = null;
		if (user_pf != null && tours != null) {					
			result = similarity.calculate(user_pf, tours);
		}
		return result;
	}
	...
	public List<Tour> getSearchTourData(String search) {
		...
		return mapper.getSearchTourData(search);
	}
	...
	public void deleteTourList(int userNo, int t_no) throws Exception {
		System.err.println("userNo " + userNo + " t_no " + t_no);
	...
	// 투어 생성하기
	@Override
	public int createTour(Tour tour, List<String> categories, String toursJson, MultipartFile tourImageFile)
			throws Exception {
		...
		for (JsonNode tourNode : rootNode) {
			...
			if (hotelNode != null) {
				...
				// 호텔 이미지 다운로드
				downloadImage(hotelPhotoUrl, "C:/DaengTrip/Front/public/images/hotel/",
...
//				ti.setT_no(t_no);											
			...
			if (placesNode != null) {
				...
				for (JsonNode placeNode : placesNode) {
					...
					System.out.println("place pk 불러오기 성공 : " + pl_no);

					// 명소 이미지 DB 저장
					downloadImage(placePhotoUrl, "C:/DaengTrip/Front/public/images/place/",
...
//					ti.setT_no(t_no);
			...
			if (resNode != null) {
				...
				for (JsonNode resItemNode : resNode) {
					...
					// 식당 이미지 DB 저장 성공
					downloadImage(resPhotoUrl, "C:/DaengTrip/Front/public/images/res/",
...
//					ti.setT_no(t_no);
	...
	public void downloadImage(String imageUrl, String folderPath, String fileName) throws Exception {
		...
		try (InputStream in = new URL(imageUrl).openStream(); // imageUrl로부터 데이터를 읽기 위한 InputStream
				ReadableByteChannel rbc = Channels.newChannel(in); // InputStream을 ReadableByteChannel로 변환하여 바이트 단위로 데이터
																	// 읽기
				FileOutputStream fos = new FileOutputStream(file)) {// 다운로드한 이미지를 로컬 파일에 저장하기 위한 FileOutputStream 생성
			fos.getChannel().transferFrom(rbc, 0, Long.MAX_VALUE); // ReadableByteChannel로부터 데이터를 읽어 FileOutputStream에
																	// 연결된 파일 채널에 기록(Long.MAX_VALUE는 가능한 최대 길이의 데이터를
																	// 전송함을 의미)
