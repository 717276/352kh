...
const reducer = (state, action) => {
    switch (action.type) {
        ...
        case DELETE_TYPES.RES:
            return {
                ...state,
                res: state.res.filter((r) => r.name !== action.payload.name),
            };
        default:
            return state;
    }
...
const Build = () => {
    ...
    function createdPlace(data) {
        setAddressHandler(data);
        dispatch({
            type: SELECT_TYPES.ADD_PLACE,
            payload: new Place(data.name, data.address, data.photo),
        });
    }
    function createdRes(data) {
        setAddressHandler(data);
        dispatch({
            type: SELECT_TYPES.ADD_RES,
            payload: new Res(data.name, data.address, data.photo),
        });
    }
        ...
        <div className="Build">
            <div className="build build_list_box">
                ...
                <div className="build_search">
                    ...
                    <img
                        className="search_btn_img"
                        src="/images/util/search.png"
            ...
            <div className="build build_selected_box">
                    ...
                    {state.hotel ? (
                        <div>
                            <div className="selected_hotel_img">
                                <img
                                    src={state.hotel.photo}
                                    onClick={() => deleteOption(0, state.hotel)}
                                />
                            </div>
                        ...
                        state.res.map((restaurante, index) => (
                            <div key={index}>
                                <div className="selected_res_img">
                                    <img
                                        src={restaurante.photo}
                                        onClick={() => deleteOption(2, restaurante)}
                                    ></img>
                                </div>
                ...
                <div className="tour_btn">
                    ...
                    <div>Day {strIdxRef.current + 1}</div>
