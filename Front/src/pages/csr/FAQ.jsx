    ...
    const handleCreateRoom = async () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                setResponse(data);
                alert(`채팅방으로 이동합니다`);
                window.open(`/csr/chat/room/${data.room_id}`, '_blank', 'width=800,height=600');
            } else {
    ...
    const deleteFaq = async (post_no) => {
        ...
        try {
            const response = await fetch(
                `http://localhost:8080/api/notice/delete/${post_no}`, // Corrected URL
                {
                    method: 'DELETE',
                    headers: {
                    ...
                    {accessToken && accessToken !== 'null' && (
                        <div>
                            <button onClick={handleCreateRoom}>
                                <img className="ChatLogoImg" src="/public/images/util/ChatLogo.png" alt="" />
                            </button>
                        </div>
                    )}
                    <h1>FAQ</h1>
                    <div>
                        <select className="faq_posts_per_page" value={postsPerPage} onChange={handlePostsPerPageChange}>
                ...
                <table className="faq_posts_table">
                    <thead>
                        <tr>
                            ...
                            <th>작성자</th>
                            <th>작성일</th>
                            {userRole === 'ROLE_ADMIN' && <th className="faq_th_delete">삭제</th>}
                        </tr>
                    </thead>
                    <tbody>
                        {filteredPosts.slice(0, postsPerPage).map((post) => (
                            <tr key={post.n_no}>
                                ...
                                {userRole === 'ROLE_ADMIN' && (
                                    <td>
                                        <button className="faq_delete_button" onClick={() => deleteFaq(post.n_no)}>
                                            삭제
                                        </button>
                                    </td>
                                )}
