...
import '../../components/css/board/ReviewList.css';
...
const ReviewList = () => {
  ...
  const commentHandler = (row) => {
    nav(`/board/review/comment/${row.ar_no}`);
  }
  const handleSortChange = (e) => {
    ...
    } else if (selectedSortType === 'bestlike') {
      sorted = [...sortedData].sort((a, b) => b.ar_like - a.ar_like);
    } else if (selectedSortType === 'worstlike') {
      sorted = [...sortedData].sort((a, b) => a.ar_like - b.ar_like);
    } else {
      sorted = [...items];
    }
    ...
    <>
      <div className="ReviewList">
        <select value={sortType} onChange={handleSortChange}>
          ...
          <option value="bestlike">좋아요 많은 순</option>
          <option value="worstlike">좋아요 적은 순</option>
        </select>
            ...
            {currentPageData.length === 0 ? (
              ...
              currentPageData.map((row) => (
                <tr key={row.ar_no}>
                  ...
                  <td>{row.t_title}</td>
                  <td onClick={() => commentHandler(row)}>{row.ar_title}</td>
                  <td>{row.ar_userId}</td>
                  <td>{row.ar_like}</td>
                  <td>{row.ar_view}</td>
        ...
        <div className="ReviewButton">
          <div className="ReviewSearch">
            ...
            <button className="primary" onClick={() => nav('/board/review/write')}>후기작성</button>
          </div>
        </div>
